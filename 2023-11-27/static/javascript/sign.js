$(function(){
    ...
    $(".ability_list").on("click", function(
        ...
        var all_check = $(".ability_list").length; // 능력 체크박스 총 갯수
        if(chk_count == all_check) //체크된 갯수와 총 갯수를 비교
            $("#all").prop("cheched", true);//체크갯수와 총 갯수가 같다면 전체 체크박스에 체크
            else
            $("#all").prop("checked", false);//체크갯수와 총갯수가 다르면 전체 체크박스에 체크해제
    });
...
$("#myphone").on("keyup", function(){
    ...
    // 때문에 객체전용 함수가 따로 존재한다.
    if( num.length>=4){
    num.splice(0,3)+"-"+num.slice(3);
    $(this).val(num);
    }if(num.length == 9){num=num.slice(0,7) + "-"+num.slice(8)}
    $(this).val
})
    $("#join").on("click", function(){
        ...
        alert(v);
         ...
         //$(".ability_list").eq(0).is("checked"); // .eq(0) 첫번째 값 , checked 체크박스에 체크되어있는 것
        // alert(chk);
        var len = $(".ability_list:checked").length;
        alert(len);
