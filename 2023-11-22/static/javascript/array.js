...
// // 배열에 저장된 데이터 삭제 - pop(마지막데이터 삭제)
// document.write("<br><br>▼삭제데이터 : " +name.pop());
// document.write("<br>" + name);
...
// // 배열의 특정위치에 추가하거나 삭제 - splice(A, B, C) A번위치에 B만큼 삭제 C에는 추가
// name.splice(2,2,"정약용", "김유신", "문익점");
// document.write("<br>" +name);
...
// document.write("<br>"+ (str.split("-")));//배열로 전환
...
function save(){
    ...
    for(var i=0; i<stat.length;i++){
        a +="<li>" + member[i] + ":" + stat[i] + "cm</li>";
        if(stat[i] < avg){avg_below.push(member[i])} 
        }
        a += "</ul>";
        a += "평균키 : " + avg +"cm";
        a +="<div> 평균키 미만 "+avg_below+"</div>";
        $("#result").html(a);
    }
    $("#name").text(member[++idx]);
    $("#stat").val("");
