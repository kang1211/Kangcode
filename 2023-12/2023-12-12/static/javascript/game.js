...
let dice1=[0,0]
let dice2=[0,0]; // 주사위 setInterval 값 저장 변수


//함수 정의
function rolling(obj){// 주사위 생성된 후 버튼을 클릭하면 주사위가 
        //이미지 변경 되는 함수, obj는 버튼 태그객체를 전달받는 변수
        $(obj).text("멈춰!");
...
function meeple_move(){ // 주사위 값에 따라 말을 움직이기
    ...
    var old_location = gamer.location; // 현재 위치(이동 전)

    // 플레이어 위치 변경
    if(gamer.location+dice_sum > 31){ //부루마블 마지막 대전위치를 넘는다면
        var diff = (gamer.location+dice_sum) - 31;
        gamer.location = diff-1;
        zone[31].func(gamer) // 출발지 통과하거나 도착하면 실행 함수, zone클래스중 31번째가 출발지 div이다.
    }else{
...
// location 매개변수는 zone 클래스들 중 몇번째 zone클래스인지 인덱스값 있음
// location의 값은 몇번째 zone클래스인지 알수도있지만, zone배열의 
// 구역객체의 인덱스로도 사용가능
    var city = zone[location];

if(city.purchase == 0){// 매입금이 0인곳은 무인도, 기금, 출발, 공항, 기금납부
    // 16-복지기금, 24-공항, 28-기금납부, 8-무인도, 0-출발지
    if(city.num != 0) //출발지 도착에 대해서는 meeple_move에서 구현했다.
        city.func(gamer);


}else if(zone[location].owner == ''){ //도시의 주인이 없는경우
    if( confirm(`${city.name}의 매입가는 ${city.purchase}만원 입니다. \n매입 하시겠습니까?`)){
        ...
        $("#pm"+gamer.num).text(gamer.money+"만원"); //변경된플레이어 금액 표시
...
function airport_move(){
    ...
    if(탑승객 !=0){ // 탑승객 변수에 0이 있다면 현재 이용할 수 있는 플레이어가 없다.
        ...
        if(클릭한도시번호 == 24){//24는 공항num
            return; // 공항을 클릭하면 아무 작업도 하지않는다.
        };
        var gamer = plyer_list[탑승객-1];
        ...
        game_todo(zone_location, gamer); // 클릭한위치에 할일 실행(매입, 기금, 무인도, 출발지 등)
        탑승객=0;
    }
}