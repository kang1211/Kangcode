...
function player(num, color){
    ...
    this.zone=0; // 매입 한 토지를 저장
    this.drift_turn=0; // 무인도 남은 턴
    this.location=0; // 현재위치
    this.파산=false; // 자금부족으로 파산한 경우 true
}
...
function overlap(location){ // 말이 생성되거나 이동했을 때 위치에 다수의 말이 있다면
...
function find_location(n){// 플레이어 말이 표시될위치 또는 이동할 위치찾기
    ...
    $(".zone").each(function(idx,item){
       var num = Number($(item).data("num"));//zone클래스 태그의 data-num값
            if(num == n){
                index=idx;
                return;// return은 해당 함수를 종료시키기도 한다.
                // each안에 만든 익병함수만 종료시키기 때문에 find_location함수를
...
function change_pcl(){//플레이어가 자신의 말 색상을 변경할경우 실행
    ...
    var num = Number($(this).attr("id").substring(3)); // attr 은 태그의 소성 - id, class, name 등등
...
$(function(){
    ...
    // 정상적으로 실행된다.
    // 비동기문제 발생!!!!
    // 비동기형태는 a=1, b=1, c=a+b를 다 동시 실행되기에 a값과 b값을 얻기 전에 c도 실행된다.
    // getJSON은 비동기 상태이다.
...
function welfare(gamer){ // 위치에 도착한 복지기금 전액 가져가기
...
//     "사회복지기금.png","우주여행.jpg","무인도.png","출발.jpg"
// ];
...
//         var image = "";
//         if(i==0)image = bg_image[0];
//         if(i==8)image = bg_image[1];
//         if(i==23)image = bg_image[2];
//         if(i==31)image = bg_image[3]
